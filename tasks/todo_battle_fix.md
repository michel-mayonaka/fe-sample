...
[progress 2025-09-28]
- 旧 `internal/ui/screens/sim_battle.go` を廃止。
- `battle.go` を新バトルシミュレータの描画に流用（レイアウト・地形切替・ログUI継承）。
- 模擬戦はコピー上で解決し永続化なし（`ui.SimulateBattleCopyWithTerrain`）。
- ステータス画面からの「戦闘へ」遷移を無効化（一覧→模擬戦ボタンのみ）。
- ステータス画面の「戦闘へ」ボタン描画を削除（非表示化）。
- シミュレーター内のログはオーバーレイで表示、Z/Enter/クリックで閉じる。
...
[progress 2025-09-28]
- 一覧画面で「模擬戦」押下→2段階選択モード（攻→防）でユニットを指定可能に。
- 一覧上でのクリック選択は廃止し、ポップアップにキャラアイコン+名前のみの一覧を表示して選択。
- シミュレータ画面に地形ボタン（平地/森/砦）を左右に配置、クリックで選択可能に。
- 互換操作として 1/2/3（攻）、Shift+1/2/3（防）も維持。
...
[progress 2025-09-28]
- 攻/反/追の各行で「〜の攻撃/反撃/追撃 → 結果」を同一行に出力。
- 戦闘終了時に勝利ログを追加（相打ち時は「相打ち」）。
- 先行/後攻の概念を追加: 1ターン目は左側が先攻、以後ターンごとに交替。ログに「ターンN 先攻: 名前」を付加。
- 自動連続実行ボタンを追加: 決着までターンを自動進行（間隔: 約10フレーム）。下部ログに進行を蓄積表示。

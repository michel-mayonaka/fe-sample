...
func TestForecastAtExplain_BreakdownMatches(t *testing.T) {
	...
	if fr != fr2 {
		t.Fatalf("Forecast mismatch: %+v vs %+v", fr, fr2)
	}
	...
	if clamp(hit, 0, 100) != bk.HitDisp {
		t.Fatalf("hit breakdown mismatch: got=%d want=%d", hit, bk.HitDisp)
	}
	...
	if bk.Dmg != wantDmg {
		t.Fatalf("dmg breakdown mismatch: got=%d want=%d", bk.Dmg, wantDmg)
	}
}

func TestTriangleRelationOf(t *testing.T) {
	if got := TriangleRelationOf("Sword", "Axe"); got != TriangleAdvantage {
		t.Fatalf("Sword>Axe should be advantage")
	}
	if got := TriangleRelationOf("Sword", "Lance"); got != TriangleDisadvantage {
		t.Fatalf("Sword<Lance should be disadvantage")
	}
	if got := TriangleRelationOf("Sword", "Sword"); got != TriangleNeutral {
		t.Fatalf("same type should be neutral")
	}
}

func TestTerrainPresetName(t *testing.T) {
	if n := TerrainPresetName(Terrain{}); n != "平地" {
		t.Fatalf("flat: %s", n)
	}
	if n := TerrainPresetName(Terrain{Avoid: 20, Def: 1}); n != "森" {
		t.Fatalf("forest: %s", n)
	}
	if n := TerrainPresetName(Terrain{Avoid: 15, Def: 2}); n != "砦" {
		t.Fatalf("fort: %s", n)
	}
	if n := TerrainPresetName(Terrain{Avoid: 1}); n != "地形" {
		t.Fatalf("other: %s", n)
	}
}
...
func TestDoubleAdvantage_Boundary(t *testing.T) {
	...
	if DoubleAdvantage(a, d) {
		t.Fatal("expected false at diff=2")
	}
	// aの体格を+1 → AS=8, 差3 → true
	a.S.Bld = 6
	if !DoubleAdvantage(a, d) {
		t.Fatal("expected true at diff=3")
	}
}

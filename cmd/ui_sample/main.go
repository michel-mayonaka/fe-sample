...
func (g *Game) runBattleRound() {
    ...
    if len(atk.Equip) > 0 {
        if wt, err := model.LoadWeaponsJSON("db/master/mst_weapons.json"); err == nil {
            if w, ok := wt.Find(atk.Equip[0].Name); ok { wepMight = w.Might }
        }
        // 使用回数を1つ消費
        if atk.Equip[0].Uses > 0 { atk.Equip[0].Uses-- }
    }
    // 命中（単純化）
    ...
    if g.userTable != nil {
        ...
        if c2, ok := g.userTable.Find(def.ID); ok {
            c2.HP = def.HP; c2.HPMax = def.HPMax

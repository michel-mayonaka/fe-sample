...
func (r *JSONInventoryRepo) Weapons() []usr.OwnWeapon {
	return append([]usr.OwnWeapon(nil), r.weapons...)
}
...
func (r *JSONInventoryRepo) Consume(id string, count int) error {
	...
	if len(id) >= 3 && id[:3] == "uw_" {
		for i := range r.weapons {
			if r.weapons[i].ID == id {
				if count > r.weapons[i].Uses {
					count = r.weapons[i].Uses
				}
	...
	if len(id) >= 3 && id[:3] == "ui_" {
		for i := range r.items {
			if r.items[i].ID == id {
				if count > r.items[i].Uses {
					count = r.items[i].Uses
				}
...
func (r *JSONInventoryRepo) Save() error {
	if err := userfs.SaveUserWeaponsJSON(r.weaponsPath, r.weapons); err != nil {
		return err
	}
	if err := userfs.SaveUserItemsJSON(r.itemsPath, r.items); err != nil {
		return err
	}
	return nil
}

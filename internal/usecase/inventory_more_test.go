...
func TestEquipWeapon_SlotExpand_NoOwner(t *testing.T) {
    ut := newUserTableForTest2(t, []user.Character{{ID: "u1", Name: "A"}})
    ur := &fakeUserRepo{t: ut}
    a := New(ur, nil, fakeInvRepo{}, nil)
    if err := a.EquipWeapon("u1", 3, "uw_x"); err != nil { t.Fatalf("EquipWeapon: %v", err) }
    c, _ := ur.Find("u1")
...
func TestEquipItem_SlotExpand_NoOwner(t *testing.T) {
    ...
    ur := &fakeUserRepo{t: ut}
    a := New(ur, nil, fakeInvRepo{}, nil)
    if err := a.EquipItem("u1", 2, "ui_x"); err != nil { t.Fatalf("EquipItem: %v", err) }
    c, _ := ur.Find("u1")

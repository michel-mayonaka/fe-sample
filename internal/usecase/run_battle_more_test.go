...
func TestRunBattleRound_NoCounter_OutOfRange(t *testing.T) {
	...
	if got[0].Equip[0].Uses != 4 {
		t.Fatalf("attacker should consume 1, got %d", got[0].Equip[0].Uses)
	}
	if got[1].Equip[0].Uses != 5 {
		t.Fatalf("defender should not consume, got %d", got[1].Equip[0].Uses)
	}
}
...
func TestRunBattleRound_NoRepos_NoPanic(t *testing.T) {
	...
	if got[0].HP == 20 && got[1].HP == 20 {
		t.Fatalf("expected some HP change")
	}
}
...
func TestRunBattleRound_InvalidSelIndex(t *testing.T) {
	...
	if ok || len(logs) != 0 || len(got) != 1 {
		t.Fatalf("should be no-op for invalid index")
	}
}

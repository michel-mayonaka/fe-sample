...
func TestEquipWeapon_TransfersOwnership(t *testing.T) {
    ...
    if err := a.EquipWeapon("u1", 0, "uw_1"); err != nil { t.Fatalf("equip weapon: %v", err) }
    c1, _ := ur.Find("u1")
    c2, _ := ur.Find("u2")
    if got := c1.Equip[0].UserWeaponsID; got != "uw_1" { t.Fatalf("u1 slot0 = %s, want uw_1", got) }
    if got := c2.Equip[0].UserWeaponsID; got != "uw_2" { t.Fatalf("u2 slot0 = %s, want uw_2 (rollback)", got) }
...
func TestEquipItem_TransfersOwnership(t *testing.T) {
    ...
    if err := a.EquipItem("u1", 1, "ui_1"); err != nil { t.Fatalf("equip item: %v", err) }
    c1, _ := ur.Find("u1")
    c2, _ := ur.Find("u2")
    if got := c1.Equip[1].UserItemsID; got != "ui_1" { t.Fatalf("u1 slot1 = %s, want ui_1", got) }
    if got := c2.Equip[1].UserItemsID; got != "ui_2" { t.Fatalf("u2 slot1 = %s, want ui_2 (rollback)", got) }
    if ur.saved == 0 { t.Fatalf("expected Save to be called") }
}

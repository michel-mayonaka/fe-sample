...
func TestRunBattleRound_UpdatesHP_Uses_Saves(t *testing.T) {
    ...
    if err != nil || !ok { t.Fatalf("RunBattleRound error=%v ok=%v", err, ok) }
    if len(logs) == 0 { t.Fatalf("expected logs") }
    // 双方1回ずつ消費想定（速度差は1）
    ...
    if ar.saved == 0 { t.Fatalf("user Save not called") }
...
func TestRunBattleRound_AttackerDouble_ConsumesTwo(t *testing.T) {
    ...
    // 攻撃側は2回、守備側は1回（反撃）
    if got[0].Equip[0].Uses != 3 { t.Fatalf("attacker uses want 3 got %d", got[0].Equip[0].Uses) }
    if got[1].Equip[0].Uses != 4 { t.Fatalf("defender uses want 4 got %d", got[1].Equip[0].Uses) }
}

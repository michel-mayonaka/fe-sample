...
func DrawStatus(dst *ebiten.Image, u uicore.Unit) {
	...
	pw, ph := float32(uicore.StatusPortraitSizePx()), float32(uicore.StatusPortraitSizePx())
	tx := int(px + pw + float32(uicore.StatusTextGapXPx()))
	ty := int(py + float32(uicore.StatusNameOffsetYPx()))
	drawStatusHeader(dst, u, int(px), int(py), int(pw), int(ph), tx, ty)
	statsTop := ty + uicore.StatusStatsTopGapPx()
	line := uicore.StatusStatsLineHPx()
	colGap := uicore.StatusStatsColGapPx()
	drawCoreStats(dst, u, tx, statsTop, line, colGap)
	wrX := tx + 2*colGap + uicore.StatusWeaponRanksXExtraPx()
	wrY := ty
	drawWeaponRanks(dst, u, wrX, wrY)
	mrX := wrX
	mrY := wrY + (4+1)*uicore.StatusRankLineHPx() + (uicore.StatusMagicRanksTopExtraPx() - 5*uicore.StatusRankLineHPx())
...
func drawStatusHeader(dst *ebiten.Image, u uicore.Unit, px, py, pw, ph, tx, ty int) {
	...
	uicore.TextDraw(dst, u.Class, uicore.FaceMain, tx, ty+uicore.StatusClassGapFromNamePx(), uicore.ColText)
	uicore.TextDraw(dst, fmt.Sprintf("Lv %d / %d    経験値 %02d / %d", u.Level, game.LevelCap, u.Exp, game.LevelUpExp), uicore.FaceMain, tx, ty+uicore.StatusLevelGapFromNamePx(), uicore.ColText)
	uicore.TextDraw(dst, fmt.Sprintf("HP %d/%d", u.HP, u.HPMax), uicore.FaceMain, tx, ty+uicore.StatusHPGapFromNamePx(), uicore.ColText)
	uicore.DrawHPBar(dst, tx, ty+uicore.StatusHPBarGapFromNamePx(), uicore.StatusHPBarWPx(), uicore.StatusHPBarHPx(), u.HP, u.HPMax)
}
...
func drawEquipList(dst *ebiten.Image, u uicore.Unit, px, py, ph int) {
	equipTitleY := py + ph + uicore.StatusEquipTitleGapYPx()
	uicore.TextDraw(dst, "装備", uicore.FaceMain, px, equipTitleY, uicore.ColAccent)
	for i := 0; i < 5; i++ {
		lineY := equipTitleY + uicore.StatusEquipLineHPx() + i*uicore.StatusEquipLineHPx()
		label := "- 空 -"
		uses := "-"
		if i < len(u.Equip) {
			it := u.Equip[i]
		...
		uicore.TextDraw(dst, label, uicore.FaceSmall, px+uicore.StatusEquipLabelGapXPx(), lineY, uicore.ColText)
		uicore.TextDraw(dst, uses, uicore.FaceSmall, px+uicore.StatusEquipUsesXPx(), lineY, uicore.ColAccent)
	}
}

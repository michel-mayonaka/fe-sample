...
func TestServiceAdapter_PressDown(t *testing.T) {
	...
	if r.Press(Confirm) || r.Down(Confirm) {
	...
	if !r.Down(Confirm) {
		t.Fatalf("expected Confirm down when pressed")
	}

	// hold
	s.SnapshotWith(ks.isDown)
	if r.Press(Confirm) {
		t.Fatalf("no press while held")
	}
	if !r.Down(Confirm) {
		t.Fatalf("down should remain true while held")
	}
	...
	s.SnapshotWith(ks.isDown)
	if r.Press(Confirm) {
		t.Fatalf("no press on release")
	}
	if r.Down(Confirm) {
		t.Fatalf("not down on release")
	}
}
...
func TestServiceAdapter_TerrainShiftMapping(t *testing.T) {
	...
	s.SnapshotWith(ks.isDown)
	if !r.Press(TerrainDef2) {
		t.Fatalf("expected TerrainDef2 on Shift+2")
	}
	if r.Press(TerrainAtt2) {
		t.Fatalf("did not expect TerrainAtt2 on Shift+2")
	}
	...
	s.SnapshotWith(ks.isDown)
	if !r.Press(TerrainAtt2) {
		t.Fatalf("expected TerrainAtt2 on 2")
	}
}

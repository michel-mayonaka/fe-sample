...
func SimulateBattleCopyWithTerrain(atk, def uicore.Unit, attT, defT gcore.Terrain, rng *rand.Rand) (uicore.Unit, uicore.Unit, []string) {
	...
	// 武器テーブル（共有/フォールバック）
	var wt = gdata.Provider().WeaponsTable()
	// 最終的に provider 前提にするが、安全のため nil チェック
	if wt == nil {
		return a, d, append(logs, "[エラー] 武器定義を読み込めませんでした")
	}
	...
	if line1 != "" {
		logs = append(logs, a.Name+" の攻撃 → "+line1)
	} else {
	...
	if d.HP > 0 && canCounter {
		...
		if line2 != "" {
			logs = append(logs, d.Name+" の反撃 → "+line2)
		} else {
	...
	if d.HP > 0 && gcore.DoubleAdvantage(ga, gd) {
		...
		if line3 != "" {
			logs = append(logs, a.Name+" の追撃 → "+line3)
		} else {
		...
		if line4 != "" {
			logs = append(logs, d.Name+" の追撃 → "+line4)
		} else {

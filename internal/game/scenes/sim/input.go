...
func (s *Sim) scHandleInput(ctx *game.Ctx) []scenes.Intent {
    ...
    if ctx != nil && ctx.Input != nil {
        ...
        if s.autoHovered && ctx.Input.Press(uinput.Confirm) { intents = append(intents, Intent{Kind: intentToggleAuto}) }
        if s.attHover >= 0 && ctx.Input.Press(uinput.Confirm) { intents = append(intents, Intent{Kind: intentSetTerrainAtt, Index: s.attHover}) }
        if s.defHover >= 0 && ctx.Input.Press(uinput.Confirm) { intents = append(intents, Intent{Kind: intentSetTerrainDef, Index: s.defHover}) }
        // キーショートカット（1/2/3, Shift+1/2/3）
        if ctx.Input.Press(uinput.TerrainAtt1) { intents = append(intents, Intent{Kind: intentSetTerrainAtt, Index: 0}) }
        if ctx.Input.Press(uinput.TerrainAtt2) { intents = append(intents, Intent{Kind: intentSetTerrainAtt, Index: 1}) }
        if ctx.Input.Press(uinput.TerrainAtt3) { intents = append(intents, Intent{Kind: intentSetTerrainAtt, Index: 2}) }
        if ctx.Input.Press(uinput.TerrainDef1) { intents = append(intents, Intent{Kind: intentSetTerrainDef, Index: 0}) }
        if ctx.Input.Press(uinput.TerrainDef2) { intents = append(intents, Intent{Kind: intentSetTerrainDef, Index: 1}) }
        if ctx.Input.Press(uinput.TerrainDef3) { intents = append(intents, Intent{Kind: intentSetTerrainDef, Index: 2}) }
    }
    return intents
}

...
func (s *Inventory) Draw(dst *ebiten.Image) {
	...
	if s.E.InvTab == 1 {
		baseI = color.RGBA{70, 100, 160, 255}
	}
...
func (s *Inventory) scHandleInput(ctx *game.Ctx) []scenes.Intent {
	...
	tx := lm + uicore.S(20)
	ty := lm + uicore.S(12)
	if geom.RectContains(mx, my, tx, ty, tabW, tabH) {
		s.tabHover = 0
	}
	tx2 := tx + tabW + uicore.S(10)
	if geom.RectContains(mx, my, tx2, ty, tabW, tabH) {
		s.tabHover = 1
	}
	...
	if ctx != nil && ctx.Input != nil {
		...
		if ctx.Input.Press(uinput.Confirm) {
			switch s.tabHover {
			case 0:
				intents = append(intents, Intent{Kind: intentTabWeapons})
			case 1:
				intents = append(intents, Intent{Kind: intentTabItems})
			}
			if s.backHovered {
				intents = append(intents, Intent{Kind: intentBack})
			}

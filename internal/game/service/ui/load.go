...
// Note: Provider 主導の経路へ移行中のため、ここはフォールバック用途として残置します。
func SampleUnit() Unit {
    // Provider/adapter が利用可能なら、それに基づく最初のユニットを返す。
    if BuildUnitsFromProviderFunc != nil {
        if us := BuildUnitsFromProviderFunc(); len(us) > 0 {
            // 代表として先頭を返す（呼び出し側で選択を保持）
            return us[0]
        }
    }
    // フォールバック: 静的サンプル

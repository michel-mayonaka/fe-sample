...
func TestRoll_All100_IncrementsAll(t *testing.T) {
    ...
    if g.HPGain != 1 { t.Fatalf("HPGain=%d", g.HPGain) }
    inc := g.Inc
...
func TestApply_ClampByClassCaps(t *testing.T) {
    ...
    gains.Inc.Str = 2
    Apply(&u, gains, 99)
    if u.Level != 11 { t.Errorf("level inc got %d", u.Level) }
    if u.HPMax > 55 { t.Errorf("HPMax clamp failed: %d", u.HPMax) }
    if u.HP > u.HPMax { t.Errorf("HP should be clamped: %d>%d", u.HP, u.HPMax) }
    if u.Stats.Str > 24 { t.Errorf("Str clamp failed: %d", u.Stats.Str) }
}

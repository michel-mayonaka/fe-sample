...
func TestPressAndDown(t *testing.T) {
	...
	if in.Press(Confirm) || in.Down(Confirm) {
	...
	in.SnapshotWith(ks.isDown)
	if !in.Press(Confirm) {
		t.Fatalf("expected Confirm press on rising edge")
	}
	if !in.Down(Confirm) {
		t.Fatalf("expected Confirm down true when pressed")
	}

	// frame 3: keep Z down -> no press, still down
	in.SnapshotWith(ks.isDown)
	if in.Press(Confirm) {
		t.Fatalf("unexpected Confirm press while held")
	}
	if !in.Down(Confirm) {
		t.Fatalf("expected Confirm down while held")
	}
	...
	in.SnapshotWith(ks.isDown)
	if in.Press(Confirm) {
		t.Fatalf("unexpected Confirm press on release")
	}
	if in.Down(Confirm) {
		t.Fatalf("unexpected Confirm down on release")
	}
}
...
func TestMenuLongPressPressOnce(t *testing.T) {
	...
	in.SnapshotWith(ks.isDown)
	if !in.Press(Menu) {
		t.Fatalf("expected Menu press on first frame")
	}
	for i := 0; i < 2; i++ {
		in.SnapshotWith(ks.isDown)
		if in.Press(Menu) {
			t.Fatalf("unexpected Menu press on hold frame %d", i)
		}
		if !in.Down(Menu) {
			t.Fatalf("expected Menu down on hold frame %d", i)
		}
	}
}
...
func TestTerrainShiftMapping(t *testing.T) {
	...
	in.SnapshotWith(ks.isDown)
	if !in.Press(TerrainDef2) {
		t.Fatalf("expected TerrainDef2 press on Shift+2")
	}
	if in.Press(TerrainAtt2) {
		t.Fatalf("did not expect TerrainAtt2 press on Shift+2")
	}
	...
	in.SnapshotWith(ks.isDown)
	if !in.Press(TerrainAtt2) {
		t.Fatalf("expected TerrainAtt2 press on 2")
	}
	if in.Press(TerrainDef2) {
		t.Fatalf("did not expect TerrainDef2 press on 2")
	}
}

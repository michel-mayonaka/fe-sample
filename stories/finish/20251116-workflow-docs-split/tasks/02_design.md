# 02_design — docs/workflow/ 構成案

## 目的
- `docs/workflow/` ディレクトリのファイル構成（ファイル名と役割）を設計し、レビュー可能な案としてまとめる。
...
## アウトプット
- `docs/workflow/` 配下の構成案（例: `overview.md`, `stories.md`, `ci.md`, `local-dev.md` など）。
...
- `docs/workflow/vibe-kanban.md`（名称は要調整候補）
...
- `docs/WORKFLOW.md` は最終的に `docs/workflow/overview.md` 相当の内容に集約し、入口としての役割に絞る想定。
- `docs/REF_STORIES.md` は `docs/workflow/stories.md` へ実体を移し、旧ファイルは「ストーリー運用ガイドは `docs/workflow/stories.md` を参照」の stub とする方針が妥当そう。
- README からの「ワークフロー/ストーリー関連」リンクは、原則 `docs/workflow/overview.md` または `docs/workflow/stories.md` のどちらかに寄せる。

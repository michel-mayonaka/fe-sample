...
## 完了条件（DoD）
- [x] docs/NAMING.md や必要に応じて docs/ARCHITECTURE.md に、`internal/game/util` の扱いと今後の方針が追記されている（少なくともメモレベル）。
...
- 2025-11-18 23:05 JST: docs/NAMING.md へケーススタディを追記し、Backlog 項目を "完了/P2" 扱いで整理。discovery（consumed）との整合を確認。

## 実施メモ（2025-11-18）
- docs/NAMING.md の「汎用名の避け方」に `internal/game/util` 撤去のケーススタディ節を追加し、再発防止とサブパッケージ命名の判断基準を具体例付きで示した。

...
- [ ] `internal/usecase/data.go` に JSON 直読みが残っていない（`rg -n 'LoadItemsJSON'` が 0）。
- [ ] `internal/repo/items.go` 相当のキャッシュ実装があり、`Reload()` で再読込可能。
- [ ] 主要経路の動作確認（在庫一覧/装備ポップアップ）。
- [ ] `make mcp` がグリーン。

## 工程（サブタスク）
- [ ] 設計: Repo/キャッシュ方針の決定 — `tasks/01_design.md`
- [ ] 実装: ItemsRepo を追加し App へ配線 — `tasks/02_impl_repo.md`
- [ ] 置換: 呼び出し箇所の更新と確認 — `tasks/03_replace_calls.md`
- [ ] 文書: API/DB_NOTES 最小更新 — `tasks/04_docs.md`
